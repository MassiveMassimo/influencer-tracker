...
// External link to the project repo, rendered as an FF ghost Button (asChild) so it
// matches the rail footer's other icon buttons — ghost hover fill, press-scale, focus
// ring. Shows a compact star count once the repo clears MIN_STARS (icon-only below it).
// lucide-react dropped brand icons, so the GitHub mark is inlined as SVG. Adapted from
// chanhdai.com/components/github-stars.
export default function GitHubLink({ className }: { className?: string }) {
  ...
  return (
    <Button asChild variant="ghost" size={showCount ? "sm" : "icon-sm"} className={className}>
      <a
        href={REPO_URL}
        target="_blank"
