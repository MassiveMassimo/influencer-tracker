...
test("rejects unknown reason and missing fields", () => {
  expect(
    validateReportBody({ handle: "h", shortcode: "a", ticker: "AAPL", reason: "spam" }),
  ).toBeNull();
  ...
  expect(
    validateReportBody({ handle: "h", shortcode: "a", ticker: "AAPL", reason: "other" }),
  ).toEqual({ handle: "h", shortcode: "a", ticker: "AAPL", reason: "other" });
});
...
test("over-long handle/shortcode/ticker rejected (bound the write)", () => {
  ...
  expect(
    validateReportBody({ handle: "h", shortcode: "a", ticker: "x".repeat(40), reason: "other" }),
  ).toBeNull();
});
