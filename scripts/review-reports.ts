...
for (const r of q) {
  ...
  const [call] = await db
    ...
    .from(calls)
    .where(
      and(eq(calls.handle, r.handle), eq(calls.shortcode, r.shortcode), eq(calls.ticker, r.ticker)),
    );
  console.log(
  ...
  if (call)
    console.log(
      `    current: ${call.ticker} buy=${call.isFirstCall} "${(call.quote ?? "").slice(0, 80)}"`,
    );
  // --target pins the override to this call within the post (required for multi-stock posts).
  console.log(
    `    fix: bun run scripts/apply-override.ts ${r.handle} ${r.shortcode} --target ${r.ticker} --reason "..." [--ticker X] [--buy false]`,
  );
}
