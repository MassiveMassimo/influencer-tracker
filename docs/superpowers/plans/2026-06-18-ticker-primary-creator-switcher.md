...
- Produces:
  ...
  Order: selected creator first (if present in `creators`), then the most-recent _other_ callers by `lastCallDate` desc (ties broken by `handle` asc), capped at `max` total. Consumed by Task 4.
...
function TickerPage() {
  ...
  const timelineCreators: TimelineCreator[] = summary.byCreator.map((b) => ({
    ...
    avatar: avatars[b.handle] ?? null,
    calls: hits
      .filter((h) => h.handle === b.handle)
      .map((h) => ({ postDate: h.postDate, isFirstCall: h.isFirstCall })),
  }));
  ...
  const callsLabel = numberFlowReady ? (
    <NumberFlow
      ...
      suffix={summary.callCount === 1 ? " call" : " calls"}
      willChange
    />
  ) : (
    ...
    <main className="mx-auto max-w-6xl space-y-6 px-4 py-8 md:px-10 md:py-10">
      ...
      <section className="overflow-hidden rounded-2xl border border-border/60 bg-background">
        ...
        <div className="border-border/40 border-t px-4 py-4 md:px-5">
          <div className="mb-3 font-mono text-[10px] text-muted-foreground uppercase tracking-[0.08em]">
            Call timeline · ★ = first call · hover to compare
          </div>
