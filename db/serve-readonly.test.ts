...
describe.skipIf(!SERVE)("serve role is read-only", () => {
  ...
  test("serve role cannot INSERT into calls", async () => {
    await expect((async () => {
      await sqlRole`INSERT INTO calls (handle, shortcode, ord, post_date, ticker, company, is_first_call, conviction, quote, returns) VALUES ('x','y',0,'2026-01-01','Z','Z',true,0,'q','{}')`;
  ...
  test("serve role cannot DELETE creators", async () => {
    await expect((async () => { await sqlRole`DELETE FROM creators WHERE handle = 'nobody'`; })())
      .rejects.toThrow(/permission denied/i);
  });
});
