...
test("a call's raw crypto ticker is scored against the canonical symbol's bars", () => {
  const ds = assembleDataset(
    { handle: "h", name: "n" },
    [cryptoCall("BTCUSD")],
    { "BTC-USD": [bar("2026-06-01", 100), bar("2026-06-08", 110)], SPY: spyBars },
    "2026-06-09",
  );
...
test("a resolvable call with no price bars is excluded", () => {
  const ds = assembleDataset(
    ...
    { SPY: spyBars },
    "2026-06-09",
  ); // ZZZZ resolves to ZZZZ but no bars present
  expect(ds.calls).toHaveLength(0);
});
...
test("an out-of-scope symbol (per the injected predicate) is excluded though it prices fine", () => {
  ...
  expect(
    assembleDataset({ handle: "h", name: "n" }, [call], bars, "2026-06-09").calls,
  ).toHaveLength(1);
  expect(
    assembleDataset(
      { handle: "h", name: "n" },
...
test("an equities-only dataset omits the crypto caveat", () => {
  const ds = assembleDataset(
    ...
    { AAPL: [bar("2026-06-01", 100), bar("2026-06-08", 110)], SPY: spyBars },
    "2026-06-09",
  );
  expect(ds.calls).toHaveLength(1);
...
test("nameFor resolver overrides company per canonical symbol; falls back to LLM company", () => {
  ...
  const calls: ReelCall[] = [
    ...
    },
    {
      ...
      postDate: "2026-06-01",
      ticker: "AMD",
      company: "AMD", // LLM-supplied, no Yahoo name
      direction: "bullish",
      isExplicitBuy: true,
      conviction: 0.8,
      quote: "q",
  ...
  const ds = assembleDataset(
    { handle: "h", name: "n" },
    calls,
    { NVDA: nvdaBars, AMD: amdBars, SPY: spyBars },
    "2026-06-09",
    undefined,
    "Reels",
    undefined,
