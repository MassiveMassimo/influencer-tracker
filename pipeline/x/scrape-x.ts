...
async function loadExisting(handle: string): Promise<TweetRecord[]> {
  try {
    return JSON.parse(await readFile(join(rawDir(handle), "tweets.json"), "utf8")) as TweetRecord[];
  } catch {
    return [];
  }
}
...
export function isTransient(e: unknown): boolean {
  ...
  return /rate.?limit|too many|429|timeout|ETIMEDOUT|ECONNRESET|socket hang up/i.test(
...
export async function scrapeX(
  ...
  if (opts.forward && records.length) {
    ...
    if (records.length) {
      console.log(
        `resuming with ${records.length} tweets; continuing older than ${windowEnd.toISOString().slice(0, 10)}`,
      );
    }
    ...
    for (; w < WINDOW_CAP; w++) {
      ...
      for (let page = 0; page < 400; page++) {
        ...
        if (records.length >= nextMilestone) {
          console.log(
            `MILESTONE ${records.length} tweets scraped (oldest ${new Date(oldest).toISOString().slice(0, 10)})`,
          );
          nextMilestone += 200;
        }
        if (!data.next || !data.list?.length) break;
      ...
      console.log(
        `scraped ${records.length} tweets so far; older than ${new Date(oldest).toISOString().slice(0, 10)}`,
      );
    }
  ...
  for (const r of records) {
    for (let i = 0; i < r.imageUrls.length; i++) {
      ...
      try {
        await downloadImage(r.imageUrls[i], dest);
      } catch (e) {
  ...
  if (incomplete)
    console.warn(`COVERAGE: hit window cap for @${user}; oldest tweets may be missing`);
