...
**Call-deletion runbook (deliberate re-score shrink).** When a re-score legitimately _drops_
...
_pre-scope_ raw bullish-buy count against the _post-scope_ committed baseline, so a >5% _post-scope_
...
`USE_DB=1` model this discard existed _because_ the DB, not `data/`, was the source of truth and
...
amortized per page, not per visit — that ISR layer _is_ the server-side cache (no KV
...
Where the UI came from, so it can be re-synced from canonical sources. **Base UI
(`@base-ui/react`) is the single primitive foundation** — Fluid Functionalism (FF)
base flavors, coss copy-in, and the raw hand-built components ALL sit on it; they are
layers on one stack, not competing stacks. Decision rule for any primitive:

1. **FF ships it → use the FF base flavor.** `@fluid` registry
   (`https://www.fluidfunctionalism.com/r/{name}.json`); base flavors publish at
   `/r/base/<name>.json` (target `components/ui/<name>.tsx`). FF adds **zero npm
   deps** — the base flavors ride the already-present `@base-ui/react` + `motion`.
2. **FF lacks it → keep the coss copy-in** (`@coss` registry,
   `https://coss.com/ui/r/{name}.json` — Cal.com's shadcn set, also Base UI).
3. **Bespoke need → build on raw `@base-ui/react/*` directly** (the 3 custom dialogs,
   the identity `navigation-menu`).
4. **One deliberate non-Base-UI exception → vaul** for every drawer (drag physics the
   Base UI drawer can't match).

`components.json` registers `@fluid`, `@coss`, and `@bklit` at the `base-nova` style.
Re-sync a primitive with `bunx --bun shadcn@latest add @<registry>/<name> --overwrite`
— FF base flavors as `@fluid/base/<name>` (NOT `@fluid/<name>-base`, which 404s).
**Re-syncing `dialog` / `accordion` / `button` re-adds a `@/lib/icon-context` import**
(FF's runtime icon-library switcher, dropped here) — re-point it to direct lucide after
any such re-sync (see Icons).

**FF's paradigms span every layer, not just its own components** (`src/lib/`):

- **Surfaces** — an 8-level elevation ladder (`bg-surface-1..8` + matching
  `shadow-surface-N`; light flattens to white after level 2 with shadow carrying
  elevation, dark is an additive gray ladder). The **semantic tokens ride it**
  (`styles.css`): `--background` = surface-1, `--card` = surface-2, `--popover` =
  surface-3 — so `bg-background` / `bg-card` / `bg-popover` _are_ the ladder; prefer
  those over literal `bg-surface-N` for page/card/popover. Rules:
  - **Base:** page = surface-1 (root `bg-surface-1`). Persistent panels + content
    cards = surface-2 — `WorkspaceRail` is `bg-surface-2`; a content card is
    `bg-card shadow-surface-2` (never `bg-background`, which equals the page and
    vanishes).
  - **Overlays elevate off their substrate via CONTEXT, not fixed classes** — this
    is the actual FF mechanism, so a popover stays visible at any depth:
    `const level = Math.min(useSurface() + OFFSET, 8)`, apply
    `surfaceClasses(level[, shadowLevel])` (`surface-classes.ts`) to the popup, and
    wrap its children in `<SurfaceProvider value={level}>` so anything opening inside
    lifts from there. Offsets: **popover / dropdown +2** (shadow pinned to 3),
    **dialog +4**. On the page a popover → surface-3; the _same_ popover inside a
    dialog → 5 + 2 = 7. Canonical examples: `ui/preview-card.tsx` + `identity-menu.tsx`
    (+2), the 3 raw dialogs (+4, below). `<Elevated>` (`elevated.tsx`) packages the
    same compute-lift-and-provide for a plain `<div>`. `useSurface` reads the
    substrate; `SURFACE_BG` / `SURFACE_SHADOW` back `surfaceClasses`.
  - **Not surfaces** (leave alone): hover / active tints (`bg-foreground/[0.0x]`),
    chart backgrounds (`--chart-*`), badges, buttons/toggles — interactive or
    decorative fills, not elevation.
- **Shape** — pill vs rounded via `useShape` / `ShapeProvider` (`shape-context.tsx`),
  consumed by `button` + `tooltip` + the tabs/nav pill families (`radius` overrides it).
- **Motion** — spring tokens in `springs.ts` (`spring`), used by `switch` + `tooltip` +
  the `table`/nav hover pills.

Two `ui/*` files stay a custom split regardless of flavor:

- `scroll-area.tsx` — the **FF base-flavor** `ScrollArea` (Base UI scrollbar) paired
  with chanhdai's pure-CSS `scroll-fade-effect` **edge fade** (see Scroll areas
  section). Keep on re-sync.
- `table.tsx` — the **FF `table`** (animated proximity-hover row pill + border-fade),
  wrapped in `ScrollArea` for the horizontal edge fade (FF ships no scroll-area). Body
  rows pass `index` to opt into the hover pill; header rows omit it (semibold). Row
  dividers use `border-border` (NOT `border-accent`, which is ~invisible on a light
  card), and fade to transparent above/below the hovered row so the pill reads as one
  block. Keep both on re-sync.
- `drawer.tsx` — vaul (drag-to-dismiss + background scale), the non-Base-UI exception
  above. FF's `mobile-drawer` (fixed-width left-nav) is inferior for both the
  bottom-sheets and MobileNav's left drawer, so it was not adopted.

**Nav, tabs & the shared pill engine.** One proximity-hover "sliding pill" engine
(`ui/indicator-track.tsx`: `useIndicatorTrack` + `IndicatorOverlays`, built on
`hooks/use-proximity-hover.ts`) backs both the tab strips and the nav/menu lists — the
highlight tracks the item nearest the cursor and animates between them. Two families
ride it:

- **`TabsList`** (`ui/tabs.tsx`, FF `tabs` on Base UI `Tabs`) — the timeframe tabs.
  Base UI owns arrow/Home/End keyboard nav (`activateOnFocus` → **auto-activate**:
  arrowing switches the tab, cheap).
- **`NavMenu`** family (`ui/nav-menu.tsx` + `nav-item`/`nav-button`/`nav-row`). `NavItem`
  = a TanStack typed-link row; `NavButton` = a `<button>` row (the pagination strip);
  `NavRow` = a `role="button"` div row for rows that open a dialog and contain inner
  `<Link>`s (proof triggers — can't nest interactives). Props: `variant` `"list"`
  (flat pill) | `"tabs"` (raised surface pill, the paginator); `orientation`
  vertical|horizontal; `radius` (pill by default; `rounded-none` for square list rows);
  and **`separated`** — draws `border-border` row dividers that fade around the hovered
  pill (via `rowSeparatorClass`; last row clears with `last:border-b-0`), so a list and
  a table share one mechanism. `separated` list surfaces: the `/` leaderboard,
  `/explore` calls, creator `CallsList`, the `/t` `CompareTable`, and the proof-viewer
  `OtherCalls`. Non-separated: `WorkspaceRail`/`RailNavList`/`RailStocks` sidebar nav,
  the paginator.

**Keyboard model.** NavMenu wires `keyboardNav` (roving tabindex, arrow keys). The
`CreatorSwitcher` (`ticker/creator-switcher.tsx`) has two: (a) the **closed tab row**
uses roving tabindex + Arrow/Home/End with **manual activation** (arrow moves focus,
Enter/Space commits) — deliberately unlike the timeframe tabs' auto-activate, because
switching creator is a route navigation, not a cheap toggle; (b) the **open combobox**
uses the `aria-activedescendant` model (focus stays on the input, arrows move the active
option, Enter follows it). The switcher is `React.memo`'d and its list/filter work is
`useMemo`'d so the ticker route's per-hover-frame re-renders don't churn it.
...
| `src/components/ui/{button,accordion,tooltip}.tsx`                                          | **FF base flavors** (`@fluid` registry) — Base UI primitives wired to FF Surfaces/Shape/Motion. Re-sync: `bunx --bun shadcn@latest add @fluid/base/<name> --overwrite` — then re-point the re-added `@/lib/icon-context` import to direct lucide (accordion + button; see Icons). **`accordion` carries a local PATCH**: `radius`/`ringRadius` props (default to the shape tokens) so a call site can square the corners (`LandingFaq` passes `rounded-none`) — re-apply after re-sync.                                                                                                                          |
| `src/components/ui/switch.tsx`                                                              | **Hand-built on FF's `@base-ui/react` Switch** — a custom drag-physics toggle (motion-value thumb, press/hover pill, pointer-drag to flick), well beyond the FF base flavor. Not a clean re-sync target; edit directly. Only consumer is `Preferences` (`SwitchRow`), which hides the built-in label via `[&>span:last-child]:sr-only` (the `Switch.Root` is itself a `<span>`, so a bare `[&>span]` would hide the whole control).                                                                                                                                                                              |
| `src/components/ui/{tabs,nav-menu,nav-item,nav-button,nav-row,indicator-track}.tsx`         | **FF `tabs` + `nav-menu` family** (`@fluid`), the shared proximity-hover pill engine — see "Nav, tabs & the shared pill engine" below                                                                                                                                                                                                                                                                                                                                                                                                                                                                            |
| `src/components/ui/{separator,card,spinner,toggle,toggle-group,preview-card,badge}.tsx`     | coss-ui (`@coss` registry) — Base UI primitives kept for gaps FF doesn't ship (no FF equivalent). Re-sync: `bunx --bun shadcn@latest add @coss/<name> --overwrite`. `badge.tsx` is dead (0 importers), left as pre-existing. (`pagination.tsx` was removed — the paginator is now a `NavMenu variant="tabs"` strip.)                                                                                                                                                                                                                                                                                             |
| 3 dialogs (`proof-viewer.tsx`, `Preferences.tsx`, `grade-detail.tsx`) + `identity-menu.tsx` | **raw `@base-ui/react/*`** (`dialog`, `navigation-menu`) — hand-composed, no ui/ wrapper. FF `DialogContent` caps at 540px (too small for proof-viewer's 768px layout), so Surfaces are wired by hand the FF way: each computes `Math.min(useSurface()+4, 8)` → `surfaceClasses(level)` on the popup/drawer + wraps content in `SurfaceProvider(level)`; `identity-menu`'s nav dropdown does the same at +2 (see Surfaces rules above)                                                                                                                                                                           |
| `src/components/ui/drawer.tsx`                                                              | vaul (kept; the one non-Base-UI exception) — mobile drag-to-dismiss + background scale                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                           |
| `src/components/ui/scroll-area.tsx`, `src/components/ui/table.tsx`                          | **FF base-flavor** `ScrollArea` (`@fluid`, Base UI scrollbar) + chanhdai `scroll-fade-effect` (chanhdai.com/components/scroll-fade-effect, `@ncdai`; CSS-only edge fade in `styles.css`). `table` wraps it. Custom split; re-sync via `@fluid/base/scroll-area`                                                                                                                                                                                                                                                                                                                                                  |
...
**`lucide-react` is the default** — both the FF base flavors and the coss copy-in use
it, wired **directly** (no `icon-context` indirection). FF's base flavors ship a runtime
icon-library switcher (`@/lib/icon-context`, pulling 6 icon packages) that's unused here,
so it was dropped: `dialog` imports `{ X }`, `accordion` `{ ChevronRight }`, `button` the
`LucideIcon` type — all by name. Reach for lucide first. **After re-syncing an FF base
flavor** (`@fluid/base/{dialog,accordion,button}`) the `@/lib/icon-context` import comes
back — re-point it to the direct lucide import.
...
- **Tooling is oxc, not biome.** Lint is `oxlint` (`bun run lint`); formatting is
  **`oxfmt`** — prettier-compatible at **printWidth 100** (`bunx prettier@3
--print-width 100` reproduces committed files byte-for-byte). Do **not** run
  `biome` — there's no biome config, so its 80-col default reflows whole files. The
  scattered `biome-ignore` comments in `src/` (mostly the bklit `charts/*` copy-ins)
  are **vestigial** — oxlint honors `oxlint-disable`/eslint-disable, not
  `biome-ignore`, so those suppressions are dead. Don't add new `biome-ignore`s.
...
- **Crossfades blur, and overlap.** Any crossfade (one element/layer swapping for
  another) animates `filter: blur()` alongside opacity — outgoing sharpens _out_
  to a few px of blur as it fades, incoming resolves _from_ blur to `blur(0)` as it
...
finfluencer accuracy — **not** a release-notes feed for the team. Describe the _benefit_
...
**When to update.** Add an entry when a _visible_ feature, improvement, or noticeable fix
...
- **Two tiers (maestri-style), chosen by the group tag:**
  ...
  - **The specific/minor stuff** — every _other_ tag (`Fixed`, `Removed`, or a custom
